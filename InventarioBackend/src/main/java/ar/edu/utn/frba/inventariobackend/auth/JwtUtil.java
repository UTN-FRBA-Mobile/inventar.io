...
/**
 * Utility component for generating and validating JSON Web Tokens (JWT) using
 * HMAC algorithms. Supports access and refresh tokens, and embeds a custom
 * claim for locationId.
 */
@Component
public class JwtUtil {
	...
	/**
	 * Generates a signed JWT with embedded claims including the username,
	 * locationId, and a flag indicating whether it's an access or refresh token.
	 *
	 * @param username
	 *            the username to set as the subject of the token.
	 * @param locationId
	 *            the location ID to embed as a custom claim.
	 * @param isRefresh
	 *            whether this is a refresh token (true) or access token (false).
	...
	public String generateToken(String username, Long locationId, boolean isRefresh) {
		...
		return Jwts.builder().subject(username).claim("locationId", locationId)
				.claim("type", isRefresh ? "refresh" : "access").issuedAt(now).expiration(expiry)
				.signWith(key, MAC_ALGORITHM).compact();
	}
	 ...
	 * 
	 * @param refreshToken
	 *            the token in which we will be based to create the new access
	 *            token.
