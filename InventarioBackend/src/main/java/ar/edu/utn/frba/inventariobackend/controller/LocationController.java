...
/**
 * REST controller for managing locations. Handles HTTP requests related to the
 * {@link Location} entity and performs actions such as retrieving, creating,
 * and verifying user locations.
 */
@RestController
...
public class LocationController {
	...
	/**
	 * Retrieves the location of the authenticated user (self) using the location ID
	 * stored in the authentication details.
	 *
	 * @return the {@link LocationResponse} representing the location of the
	 *         authenticated user
	 * @throws ResponseStatusException
	 *             if the authentication details are not found or invalid (HTTP 403
	 *             Forbidden)
	 */
	...
	/**
	 * Verifies if the provided latitude and longitude match the location of the
	 * authenticated user.
	 *
	 * @param verifySelfRequest
	 *            the request containing latitude and longitude to be verified
	 * @return {@code true} if the provided coordinates match the authenticated
	 *         user's location, {@code false} otherwise
	 */
	@SecurityRequirement(name = "bearerAuth")
	@GetMapping("/verify")
	public boolean verifySelfLocation(@Valid @NotNull VerifySelfRequest verifySelfRequest) {
		Optional<LocationResponse> location = locationService.getLocationByPosition(verifySelfRequest.latitude(),
				verifySelfRequest.longitude());
