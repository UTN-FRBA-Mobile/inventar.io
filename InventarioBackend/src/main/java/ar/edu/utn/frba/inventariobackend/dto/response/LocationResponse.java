...
public record LocationResponse(long id, double latitude, double longitude, double radius, String name) {

	/**
	 * Creates a {@link LocationResponse} from a {@link Location} domain object.
	 *
	 * @param location
	 *            the {@link Location} entity to convert.
	 * @return a new {@link LocationResponse} instance with data copied from the
	 *         entity.
	 */
	public static LocationResponse fromLocation(Location location) {
		return new LocationResponse(location.getId(), location.getLatitude(), location.getLongitude(),
				location.getRadius(), location.getName());
	}
}
