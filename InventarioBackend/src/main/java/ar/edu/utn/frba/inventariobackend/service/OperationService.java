...
public class OperationService {
	 ...
	 *
	 * @param request
	 *            The DTO containing details for the new order and its items.
	 * @return An {@link OrderResponse} representing the newly created order.
	 * @throws NoSuchElementException
	 *             if any product ID in the order items is not found.
	 */
	@Transactional
	public OrderResponse createOrder(OrderCreationRequest request) {
		Order order = new Order(Status.PENDING, null, request.scheduledDate(), null, request.idLocation(),
				request.sender());

		Order pushedOrder = orderRepository.save(order);

		var itemsByOperation = request.productAmount().entrySet().stream().map(
				entry -> new ItemByOperation(pushedOrder.getId(), entry.getKey(), ItemType.ORDER, entry.getValue()))
				.toList();

		itemByOperationRepository.saveAll(itemsByOperation);

		return OrderResponse.fromOrder(pushedOrder, request.productAmount(), getProductNames(request.productAmount()));
	}
	 ...
	 *
	 * @param request
	 *            The DTO containing details for the new shipment and its items.
	 * @return A {@link ShipmentResponse} representing the newly created shipment.
	 * @throws NoSuchElementException
	 *             if any product ID in the shipment items is not found.
	 */
	@Transactional
	public ShipmentResponse createShipment(ShipmentCreationRequest request) {
		Shipment shipment = new Shipment(Status.PENDING, null, null, request.idLocation(), request.customerName());

		Shipment pushedShipment = shipmentRepository.save(shipment);

		var itemsByOperation = request.productAmount().entrySet().stream()
				.map(entry -> new ItemByOperation(pushedShipment.getId(), entry.getKey(), ItemType.SHIPMENT,
						entry.getValue()))
				.toList();
		...
		return ShipmentResponse.fromShipment(shipment, request.productAmount(),
				getProductNames(request.productAmount()));
	}
	 ...
	 *
	 * @param locationId
	 *            The ID of the location to filter orders by.
	...
	public List<OrderResponse> getAllOrders(Long locationId) {
		...
		return orderRepository.findAllByIdLocation(locationId).stream().map(this::getOrderResponse)
	 ...
	 *
	 * @param locationId
	 *            The ID of the location to filter shipments by.
	...
	public List<ShipmentResponse> getAllShipments(Long locationId) {
		...
		return shipmentRepository.findAllByIdLocation(locationId).stream().map(this::getShipmentResponse)
				.collect(Collectors.toList());
	}
	 ...
	 * <p>
	 * This method ensures thread-safe execution per shipment ID, validates the
	 * shipment's {@link Status#PENDING} state, and checks for sufficient stock at
	 * the shipment's location. The shipment's status is updated to
	 * {@link Status#IN_PROGRESS} if stock is available, or to
	 * {@link Status#BLOCKED} otherwise.
	 * </p>
	 *
	 * @param id
	 *            The unique identifier of the shipment to be started.
	 * @return A {@link ShipmentResponse} reflecting the updated shipment status.
	 * @throws NoSuchElementException
	 *             If the shipment is not found.
	 * @throws IllegalStateException
	 *             If the shipment is not pending or if there is insufficient stock.
	 */
	...
	public ShipmentResponse startShipment(long id) {
		...
		synchronized (lockObject) {
			...
			boolean enoughStock = requiredStock.entrySet().stream()
	 ...
	 * <p>
	 * This method ensures thread-safe execution per order ID and validates the
	 * order's {@link Status#PENDING} state. The order's status is updated to
	 * {@link Status#PENDING}.
	 * </p>
	 *
	 * @param id
	 *            The unique identifier of the order to be started.
	 ...
	 * <p>
	 * Updates shipment status to {@link Status#COMPLETED} and deducts required
	 * stock.
	 * </p>
	 *
	 * @param id
	 *            The unique identifier of the shipment to complete.
	 * @return A {@link ShipmentResponse} for the completed shipment.
	 * @throws NoSuchElementException
	 *             If the shipment is not found.
	 * @throws IllegalStateException
	 *             If the shipment is not in {@link Status#IN_PROGRESS}.
	 */
	@Transactional
	 ...
	 * <p>
	 * Updates order status to {@link Status#COMPLETED} and deducts stock using the
	 * provided `processedStock` quantities.
	 * </p>
	 *
	 * @param id
	 *            The unique identifier of the order to complete.
	 * @param processedStock
	 *            Map of product IDs to quantities to deduct from stock.
	 * @return An {@link OrderResponse} for the completed order.
	 * @throws NoSuchElementException
	 *             If the order is not found.
	 * @throws IllegalStateException
	 *             If the order is not in {@link Status#IN_PROGRESS}.
	 */
	@Transactional
	 ...
	 * <p>
	 * Updates the shipment's status to {@link Status#BLOCKED} without performing
	 * any stock operations. This indicates that the shipment's processing has been
	 * halted.
	 * </p>
	 *
	 * @param id
	 *            The unique identifier of the shipment to block.
	 * @return A {@link ShipmentResponse} for the blocked shipment.
	 * @throws NoSuchElementException
	 *             If the shipment is not found.
	 * @throws IllegalStateException
	 *             If the shipment cannot be blocked from its current status.
	 */
	@Transactional
	 ...
	 * <p>
	 * Updates the shipment's status to {@link Status#PENDING}. This indicates that
	 * the shipment's ready to be picked up.
	 * </p>
	 *
	 * @param id
	 *            The unique identifier of the shipment to unblock.
	 * @return A {@link ShipmentResponse} for the unblocked shipment.
	 * @throws NoSuchElementException
	 *             If the shipment is not found.
	 * @throws IllegalStateException
	 *             If the shipment cannot be unblocked from its current status.
	 */
	@Transactional
	 ...
	 * @param itemType
	 *            The type of the item which can be ORDER or SHIPMENT.
	...
	private Map<Long, Integer> getProductAmount(long operationId, ItemType itemType) {
		return itemByOperationRepository.findAllByIdOperationAndItemType(operationId, itemType).stream()
	 ...
	 *
	 * @param order
	 *            The order to generate the response from.
	 ...
	 *
	 * @param shipment
	 *            The shipment to generate the response from.
