...
public class OperationController {
	 ...
	 *            The request DTO containing details for the new order.
	 * @return A {@link ResponseEntity} containing the {@link OrderResponse} of the
	 *         newly created order and HTTP status 201 (Created).
	 */
	@PostMapping("/orders")
	 ...
	 *
	 * @param shipmentCreationRequest
	 *            The request DTO containing details for the new shipment.
	 * @return A {@link ResponseEntity} containing the {@link ShipmentResponse} of
	 *         the newly created shipment and HTTP status 201 (Created).
	 */
	@PostMapping("/shipments")
	public ResponseEntity<ShipmentResponse> createShipment(
			@Valid @RequestBody ShipmentCreationRequest shipmentCreationRequest) {
	 ...
	 *         {@link ShipmentResponse} objects and HTTP status 200 (OK).
	 */
	@SecurityRequirement(name = "bearerAuth")
	 ...
	 * <p>
	 * This endpoint requires authentication with a bearer token. When invoked, it
	 * delegates the business logic to the {@code operationService} to begin the
	 * processing of the shipment identified by the provided ID.
	 * </p>
	 *
	 * @param id
	 *            The unique identifier of the shipment to be started.
	 * @return A {@link ResponseEntity} containing a {@link ShipmentResponse} object
	 *         that represents the outcome or details of the started shipment
	 *         operation. The HTTP status code will reflect the success or failure
	 *         of the operation.
	 */
	@SecurityRequirement(name = "bearerAuth")
	 ...
	 * This endpoint requires authentication with a bearer token. When invoked, it
	 * delegates the business logic to the {@code operationService} to begin the
	 * processing of the order identified by the provided ID.
	 * </p>
	 *
	 * @param id
	 *            The unique identifier of the order to be started.
	 * @return A {@link ResponseEntity} containing an {@link OrderResponse} object
	 *         that represents the outcome or details of the started order
	 *         operation. The HTTP status code will reflect the success or failure
	 *         of the operation.
	 */
	@SecurityRequirement(name = "bearerAuth")
	 ...
	 * <p>
	 * This endpoint requires authentication with a bearer token. It delegates the
	 * finishing logic to the {@code operationService}.
	 * </p>
	 *
	 * @param id
	 *            The unique identifier of the shipment to be finished.
	 * @return A {@link ResponseEntity} containing:
	 *         <ul>
	 *         <li>{@code 200 OK} with a {@link ShipmentResponse} if the shipment is
	 *         successfully finished.</li>
	 *         <li>{@code 404 Not Found} if no shipment with the given ID
	 *         exists.</li>
	 *         <li>{@code 400 Bad Request} with an error message if the shipment
	 *         cannot be finished due to its current state or business rules.</li>
	 *         </ul>
	 */
	@SecurityRequirement(name = "bearerAuth")
	...
	/**
	 * Finishes the processing of a specific order, including details about
	 * processed stock.
	 * <p>
	 * This endpoint requires authentication with a bearer token. It processes the
	 * order and the stock quantities associated with its completion, delegating the
	 * logic to the {@code operationService}.
	 * </p>
	 *
	 * @param id
	 *            The unique identifier of the order to be finished.
	 * @param processedStock
	 *            A {@code Map} where keys are product IDs ({@code Long}) and values
	 *            are the quantities ({@code Integer}) of those products that were
	 *            processed/fulfilled as part of finishing this order. This body is
	 *            subject to validation.
	 * @return A {@link ResponseEntity} containing:
	 *         <ul>
	 *         <li>{@code 200 OK} with an {@link OrderResponse} if the order is
	 *         successfully finished.</li>
	 *         <li>{@code 404 Not Found} if no order with the given ID exists.</li>
	 *         <li>{@code 400 Bad Request} with an error message if the order cannot
	 *         be finished due to its current state, business rules, or if the
	 *         {@code processedStock} payload is invalid.</li>
	 *         </ul>
	 */
	@SecurityRequirement(name = "bearerAuth")
	 ...
	 * <p>
	 * This endpoint requires authentication with a bearer token. It marks the
	 * shipment as blocked, delegating the status update logic to the
	 * {@code operationService}.
	 * </p>
	 *
	 * @param id
	 *            The unique identifier of the shipment to be blocked.
	 * @return A {@link ResponseEntity} containing:
	 *         <ul>
	 *         <li>{@code 200 OK} with a {@link ShipmentResponse} if the shipment is
	 *         successfully blocked.</li>
	 *         <li>{@code 404 Not Found} if no shipment with the given ID
	 *         exists.</li>
	 *         <li>{@code 400 Bad Request} with an error message if the shipment
	 *         cannot be blocked due to its current state or business rules.</li>
	 *         </ul>
	 */
	@SecurityRequirement(name = "bearerAuth")
	 ...
	 * <p>
	 * This endpoint requires authentication with a bearer token. It marks the
	 * shipment as pending, delegating the status update logic to the
	 * {@code operationService}.
	 * </p>
	 *
	 * @param id
	 *            The unique identifier of the shipment to be unblocked.
	 * @return A {@link ResponseEntity} containing:
	 *         <ul>
	 *         <li>{@code 200 OK} with a {@link ShipmentResponse} if the shipment is
	 *         successfully unblocked.</li>
	 *         <li>{@code 404 Not Found} if no shipment with the given ID
	 *         exists.</li>
	 *         <li>{@code 400 Bad Request} with an error message if the shipment
	 *         cannot be unblocked due to its current state or business rules.</li>
	 *         </ul>
	 */
	@SecurityRequirement(name = "bearerAuth")
