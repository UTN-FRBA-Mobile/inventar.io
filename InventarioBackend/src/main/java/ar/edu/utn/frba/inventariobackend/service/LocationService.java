 ...
 * <p>
 * Provides methods for retrieving and creating locations, acting as an
 * intermediary between the controller and repository.
 */
@Service
...
public class LocationService {
	 ...
	 * @return an Optional of {@link LocationResponse} which will provide the
	...
	public Optional<LocationResponse> getLocationByPosition(Double latitude, Double longitude) {
		return locationRepository.findAll().stream().map(location -> {
			double distanceToReference = calculateDistance(Objects.requireNonNull(latitude),
					Objects.requireNonNull(longitude), location.getLatitude(), location.getLongitude());

			return Pair.of(location, distanceToReference);
		}).filter(pair -> pair.getValue() <= pair.getKey().getRadius())
				.sorted(Comparator.comparingDouble(Pair::getValue))
				.map(pair -> LocationResponse.fromLocation(pair.getKey())).findFirst();
	}
	 ...
	 * @return the {@link LocationResponse} representing the location with the
	 *         specified ID
	 * @throws NoSuchElementException
	 *             if no location with the given ID is found
	 */
	 ...
	 *            the request containing location creation data
	...
	/**
	 * Calculates the distance in meters between two points on Earth using the
	 * Haversine formula.
	 *
	 * @param originLat
	...
	private double calculateDistance(double originLat, double originLon, double targetLat, double targetLon) {
		...
		double haversineOfCentralAngle = Math.pow(Math.sin(deltaLatRadians / 2), 2)
				+ Math.pow(Math.sin(deltaLonRadians / 2), 2) * Math.cos(originLatRadians) * Math.cos(targetLatRadians);
